...
def evaluate(opt):
    ...
    txt = open(os.path.join(load_weights_folder, "evaluate_results.txt"), "w+")

    gt_path = os.path.join(splits_dir, opt.eval_split, "gt_depths.npz")
    gt_depths = np.load(gt_path, fix_imports=True, encoding='latin1', allow_pickle=True)["data"]

    for e in range(0, num_epochs):
        folder = os.path.join(load_weights_folder, 'models', 'weights_{}'.format(e))
        ...
        if opt.ext_disp_to_eval is None:
            ...
            assert os.path.isdir(folder), \
                "Cannot find a folder at {}".format(folder)

            if e == 0:
                print("-> Loading weights from {}".format(load_weights_folder), file=txt)
                print("-> Loading weights from {}".format(load_weights_folder))

            filenames = readlines(os.path.join(splits_dir, opt.eval_split, "test_files.txt"))
            encoder_path = os.path.join(folder, "encoder.pth")
            decoder_path = os.path.join(folder, "depth.pth")
            ...
            if e == 0:
                print("-> Computing predictions with size {}x{}".format(
                    encoder_dict['width'], encoder_dict['height']), file=txt)
                print("-> Computing predictions with size {}x{}".format(
            ...
            if e == 0:
                print("-> Loading predictions from {}".format(opt.ext_disp_to_eval), file=txt)
        ...
        if opt.save_pred_disps:
            output_path = os.path.join(
                folder, "disps_{}_split.npy".format(opt.eval_split))
            if i == 0:
        ...
        if e == 0:
            print("-> Evaluating", file=txt)
            print("-> Evaluating")
            print("   Mono evaluation - using median scaling", file=txt)
        ...
        if e == 0:
            print("\n  " + ("{:>8} | " * 7).format("abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"), file=txt)
            print("\n  " + ("{:>8} | " * 7).format("abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"))

        print(("&{: 8.3f}  " * 7).format(*mean_errors.tolist()) + "\\\\" + "  weights_%d" % e, file=txt)
        print(("&{: 8.3f}  " * 7).format(*mean_errors.tolist()) + "\\\\" + "  weights_%d" % e)

        if e == num_epochs - 1:
            print("\n\n  " + ("flops: {0}, params: {1}, flops_e: {2}, params_e:{3}, flops_d:{4}, params_d:{5}").format(flops, params, flops_e, params_e, flops_d, params_d), file=txt)
